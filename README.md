...
- **List Pipe Accessories** places and labels one sample of every loaded Pipe Accessory family type at a 0'-0" offset from the active plan's level in two floor-plan catalogs: a Fine (double-line) view and a Coarse (single-line) view. Both hide unrelated model categories.
